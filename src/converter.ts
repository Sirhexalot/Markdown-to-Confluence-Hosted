...
export async function convertWordFile(inputPath: string, options: ConvertWordOptions = {}) {
  ...
  try {
    if (extension === ".docx") {
      ...
      await importLegacyDocToMarkdown(
        pandocCommand,
        inputPath,
        outputPath,
        tempDirectory,
...
async function exportLegacyDoc(
  pandocCommand: string,
  inputPath: string,
  outputPath: string,
  workingDirectory: string,
  configuredLibreOfficePath: string
) {
  ...
  if (!converter) {
    throw new Error(
      "Legacy .doc export requires macOS textutil or LibreOffice/soffice. On this system, use docx or install a converter."
    );
  }
...
async function importLegacyDocToMarkdown(
  pandocCommand: string,
  inputPath: string,
  outputPath: string,
  workingDirectory: string,
...
async function convertDocToDocx(
  ...
  if (!converter) {
    throw new Error(
      "Importing legacy .doc requires LibreOffice or soffice. Install it or convert the file to .docx first."
    );
  }
...
async function extractHtmlFromMhtmlDoc(inputPath: string, workingDirectory: string) {
  ...
  const htmlMatch = rawContent.match(
    /Content-Type:\s*text\/html[\s\S]*?Content-Transfer-Encoding:\s*quoted-printable[\s\S]*?\r?\n\r?\n([\s\S]*?)(?:\r?\n--[-=_A-Za-z0-9.]+--?|\s*$)/
  );
...
function sanitizeConfluenceHtml(input: string) {
  return input
    .replace(
      /(<a\b[^>]*class="[^"]*\bconfluence-userlink\b[^"]*"[^>]*>[\s\S]*?<\/a>)(?=\S)/gi,
      "$1 "
    )
    .replace(
      /<a\b[^>]*class="[^"]*\bconfluence-userlink\b[^"]*"[^>]*>([\s\S]*?)<\/a>/gi,
      (_, text: string) => stripHtmlTags(text).replace(/\s+/g, " ").trim()
    )
    ...
    .replace(
      /<(td|th)>\s*<ul>\s*<li>([\s\S]*?)<\/li>\s*<\/ul>\s*<\/\1>/gi,
      "<$1>$2</$1>"
    )
...
async function convertPandocDocumentToMarkdown(
  pandocCommand: string,
  inputFormat: "docx" | "html" | "rtf",
  inputPath: string,
  outputPath: string
) {
...
function convertHtmlTablesToMarkdown(input: string) {
...
function htmlTableToMarkdown(tableHtml: string) {
  ...
  const rows = rowMatches
    ...
    .filter((row): row is { isHeader: boolean; cells: string[] } => Boolean(row));
...
function normalizeHtmlTableCell(input: string) {
  const listNormalized = input
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/p>\s*<p\b[^>]*>/gi, "\n\n")
    .replace(/<(ul|ol)\b[^>]*>\s*([\s\S]*?)<\/\1>/gi, (_match, listType: string, listBody: string) => {
      const items = [...listBody.matchAll(/<li\b[^>]*>([\s\S]*?)<\/li>/gi)].map((itemMatch) =>
        decodeBasicHtmlEntities(stripHtmlTags(itemMatch[1])).replace(/\s+/g, " ").trim()
      );
...
async function isRtfBackedDoc(inputPath: string) {
  const rawContent = await fs.readFile(inputPath, "utf8").catch(() => undefined);
  if (!rawContent) {
    return false;
  }
...
async function resolveDocConverter(configuredLibreOfficePath: string) {
