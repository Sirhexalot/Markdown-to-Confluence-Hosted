...
async function convertMarkdownToWikiMarkupFile(inputPath, options = {}) {
    if (path.extname(inputPath).toLowerCase() !== ".md") {
        throw new Error("Please select a Markdown file.");
    }
    const inputDirectory = path.dirname(inputPath);
...
function normalizeGeneratedMarkdown(input) {
    return normalizeHeadingLevels(normalizeMarkdownTables(convertHtmlTablesToMarkdown(input)
        .replace(/\r\n/g, "\n")
        .replace(/^\s*•\s+/gm, "- ")
        .replace(/[ \t]*\\[ \t]*(?=\n)/g, "")
        .replace(/^[ \t]*\\[ \t]*$/gm, "")
...
function convertMarkdownToWikiMarkup(input) {
...
function convertMarkdownLineToWiki(line) {
    if (/^\s*$/.test(line)) {
        return "";
    }
...
function convertMarkdownInlineToWiki(input) {
    const boldSegments = [];
    const codeSegments = [];
    const imageSegments = [];
    let converted = input
    ...
    converted = converted.replace(/!\[([^\]]*)\]\((\S+?)(?:\s+"[^"]*")?\)/g, (_match, altText, source) => {
    ...
    converted = converted.replace(/<span\b[^>]*style=["'][^"']*color\s*:\s*([^;"']+)[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi, (_match, color, text) => `{color:${color.trim()}}${convertMarkdownInlineToWiki(text)}{color}`);
    converted = converted
        ...
        .replace(/(^|[\s([<{])_([^_\n]+)_($|[\s)\]>}.,!?;:])/g, "$1_$2_$3");
    converted = converted
        .replace(/@@BOLD(\d+)@@/g, (_match, index) => boldSegments[Number(index)] ?? "")
        .replace(/@@IMAGE(\d+)@@/g, (_match, index) => imageSegments[Number(index)] ?? "")
        .replace(/@@CODE(\d+)@@/g, (_match, index) => codeSegments[Number(index)] ?? "")
        .replace(/@@ESCAPED_PIPE@@/g, "|");
    return converted.trimEnd();
}
function convertMarkdownTableCellToWiki(input) {
    const converted = convertMarkdownInlineToWiki(input);
    return converted.length > 0 ? converted : " ";
}
function normalizeHeadingLevels(input) {
    const lines = input.split("\n");
    let previousLevel = 0;
    for (let index = 0; index < lines.length; index += 1) {
        ...
        const normalizedLevel = previousLevel === 0 ? currentLevel : Math.min(currentLevel, previousLevel + 1);
...
function parseFencedCodeBlock(lines, startIndex) {
    ...
    while (currentIndex < lines.length) {
        ...
        const closingFenceMatch = new RegExp(`^${indentation}${fenceCharacter}{${fenceLength},}\\s*$`).exec(candidate);
